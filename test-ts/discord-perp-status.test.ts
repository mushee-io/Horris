...
const position: HorrisUpDownPosition = {
  market: "BTC/USDT", marketToken: btc.marketToken, collateralToken: "0xd96a1ac57a180a3819633bCE3dC602Bd8972f595",
...
function order(orderType: number, sizeUsd: string, isFrozen = false): HorrisUpDownOrder {
  return {
    key: `0x${"11".repeat(32)}`, market: "BTC/USDT", marketToken: btc.marketToken, type: `OrderType(${orderType})`, orderType,
    side: "long", sizeUsd, collateralAmount: "0", triggerPrice: null, acceptablePrice: null, executionFeeCelo: "1",
    updatedAt: 0, validFrom: 0, isFrozen, autoCancel: true,
  };
}
...
describe("Discord perp status formatter", () => {
  ...
  it("reports pending entries before a live position exists", () => {
    const text = formatDiscordPerpStatus([], [order(2, "100")]);
    expect(text).toContain("1 pending entry");
  });

  it("reports orphan stops after a matching position disappears", () => {
    const text = formatDiscordPerpStatus([], [order(6, "100")]);
    expect(text).toContain("1 orphan stop");
  });
  ...
  it("reports stop coverage and protection phase", () => {
    ...
    expect(text).toContain("Risk alerts: 1 critical");
  ...
  it("caps the number of position lines and total Discord response length", () => {
    const positions = Array.from({ length: 8 }, (_, index) => ({
      ...position, market: `TEST${index}/USDT`, marketToken: `0x${(index + 1).toString(16).padStart(40, "0")}` as `0x${string}`,
    }));
