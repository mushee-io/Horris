...
export function analyzePerpRiskState(
  ...
  const protections = positions.map((position) => {
    ...
    const pendingIncreaseUsd = orders
      .filter((order) => order.marketToken.toLowerCase() === position.marketToken.toLowerCase() && order.side === position.side && (order.orderType === 2 || order.orderType === 3 || order.orderType === 8))
    ...
    if (!fullyStopProtected) {
      alerts.push({
        ...
        message: `${position.market} ${position.side} has only ${stopCoveragePercent.toFixed(1)}% active stop-loss coverage.`,
      });
    }
    if (frozenStops.length) {
      alerts.push({
        ...
        code: "FROZEN_STOP",
        market: position.market,
        message: `${position.market} has ${frozenStops.length} frozen stop-loss order${frozenStops.length === 1 ? "" : "s"}.`,
      });
    }
    if (!leverageWithinPolicy) {
      alerts.push({
        severity: "warning",
        code: "LEVERAGE_POLICY",
        market: position.market,
    ...
    if (pendingIncreaseUsd > 0) {
      alerts.push({
        severity: "warning",
        code: "PENDING_INCREASE",
        market: position.market,
        message: `${position.market} has $${pendingIncreaseUsd.toFixed(2)} of additional pending increase exposure.`,
      });
    }
    ...
    return {
      ...
      leverageWithinPolicy,
      stopOrderCount: stops.length,
      stopCoveragePercent,
      fullyStopProtected,
      hasFrozenStop: frozenStops.length > 0,
      pendingIncreaseUsd,
    };
  });

  for (const order of orders.filter((item) => item.isFrozen && item.orderType !== 6)) {
    alerts.push({
      severity: "warning",
      code: "FROZEN_ORDER",
      market: order.market,
      message: `${order.market} ${order.type} order is frozen.`,
    });
  }
