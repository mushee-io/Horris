...
const critical = (reason: string): ProtectionRecovery => ({ severity: "critical", freezeNewRisk: true, retryProtection: false, recommendReduceOrClose: true, humanReviewRequired: true, reason });

export function planProtectionFailureRecovery(input: ProtectionFailureInput): ProtectionRecovery {
  if (!Number.isFinite(input.stopCoveragePercent) || input.stopCoveragePercent < 0 || input.stopCoveragePercent > 100) return critical("Protection coverage telemetry is invalid. Treat protection state as unknown and fail closed.");
  if (!Number.isSafeInteger(input.retryCount) || input.retryCount < 0) return critical("Protection retry telemetry is invalid. Do not retry automatically.");
  ...
  if (!input.oracleFresh || !input.rpcHealthy) return critical("Protection cannot be trusted while oracle or RPC state is unhealthy. Freeze new risk and prepare a reviewed reduce/close recovery.");
  if (input.protectionOrderFrozen) return critical("Protection order is frozen. Do not stack replacement orders until the stale order is reviewed/cancelled.");
  ...
  return critical("Protection retries are exhausted. Stop adding risk and prepare a reviewed reduce/close transaction.");
}
