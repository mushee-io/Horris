...
export type ExecutionActivity = { txHash: Hash; blockNumber: bigint; adapter: Address; assetIn: Address; assetOut: Address; amountIn: bigint; amountOut: bigint; slippageBps: number };
...
const executionEvent = parseAbiItem("event ExecutionCompleted(address indexed adapter,address indexed assetIn,address indexed assetOut,uint256 amountIn,uint256 amountOut,uint16 slippageBps)");
...
export async function getVaultActivity(client: HorrisReadClient, vault: Address, fromBlock: bigint): Promise<ExecutionActivity[]> {
  ...
  return logs.map((log: any) => ({
    ...
    assetOut: log.args.assetOut as Address,
