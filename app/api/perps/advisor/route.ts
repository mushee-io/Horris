...
export async function POST(request: NextRequest) {
  try {
    if (isCrossSiteBrowserRequest(request)) {
      return json({ error: "Cross-site advisor requests are not allowed", executionEnabled: false }, 403);
    }

    const limit = checkBurstRateLimit(request, { namespace: "horris-ai-advisor", maxPerIp: 10, maxGlobal: 25, windowMs: 60_000 });
    if (!limit.allowed) {
      return json(
        { error: "Horris AI request limit reached. Try again shortly.", code: "AI_RATE_LIMITED", executionEnabled: false },
        429,
    ...
    if (!body || typeof body !== "object" || Array.isArray(body)) return json({ error: "Invalid request body", executionEnabled: false }, 400);
    const input = body as Record<string, unknown>;
    if (Object.keys(input).some((key) => !allowedFields.has(key))) return json({ error: "Unexpected advisor request field", executionEnabled: false }, 400);
    ...
    if (!marketMeta) return json({ error: "Unsupported UpDown market", executionEnabled: false }, 400);
    if (!sides.includes(side)) return json({ error: "Side must be long or short", executionEnabled: false }, 400);
    if (!risks.includes(risk)) return json({ error: "Invalid Horris risk profile", executionEnabled: false }, 400);
    ...
    if (!Number.isFinite(accountBalanceUsd) || accountBalanceUsd <= 0 || accountBalanceUsd > 1_000_000_000 || !Number.isFinite(entryPrice) || entryPrice <= 0 || entryPrice > 1_000_000_000) {
      return json({ error: "Valid positive account balance and entry price are required", executionEnabled: false }, 400);
    }
    ...
    if (preferredMarginUsd !== undefined && (!Number.isFinite(preferredMarginUsd) || preferredMarginUsd <= 0 || preferredMarginUsd > 1_000_000_000)) {
      return json({ error: "Invalid preferred margin", executionEnabled: false }, 400);
    }
    if (preferredLeverage !== undefined && (!Number.isFinite(preferredLeverage) || preferredLeverage <= 0 || preferredLeverage > 1_000)) {
      return json({ error: "Invalid preferred leverage", executionEnabled: false }, 400);
    }
