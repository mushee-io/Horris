...
export default function Home() {
  ...
  async function connectWallet() {
    if (!window.ethereum) {
      setStatus("Install MetaMask or another EVM wallet first");
      return;
    }
    try {
  ...
  async function reviewStrategy() {
    ...
    try {
      ...
      setStatus(`Policy passed · live route found for ${amount} USDC`);
    } catch (error: any) {
      setQuote("");
  ...
  async function executeStrategy() {
    ...
    setBusy(true);
    setStatus("Preparing policy-compliant transaction…");
    try {
      await ensureCeloSepolia();
      ...
      setTxHash(hash);
      setStatus("Transaction submitted · waiting for Celo confirmation…");
      await publicClient.waitForTransactionReceipt({ hash });
      await refreshBalance(address);
      setStatus("Execution confirmed on Celo Sepolia ✓");
    } catch (error: any) {
  ...
  return (
    <main>
      <nav className="nav shell">
        <div className="brand">HORRIS<span>.</span></div>
        <div className="navlinks"><a href="#engine">Engine</a><a href="#policy">Policy</a><a href="#activity">Activity</a></div>
        <button className="button button-small" onClick={connectWallet}>{address ? `${address.slice(0, 6)}…${address.slice(-4)}` : "Connect wallet"}</button>
      </nav>

      <section className="hero shell">
        <div><p className="eyebrow">AI EXECUTION INFRASTRUCTURE · CELO</p><h1>Intent in.<br />Policy checked.<br /><em>Execution out.</em></h1><p className="lede">Horris turns a strategy proposal into a transparent Celo transaction without giving the agent unrestricted authority.</p></div>
        <div className="network-card"><span className="dot" /> CELO SEPOLIA<strong>Live testnet execution</strong><small>First adapter: Mento · USDC → USDm. Testnet assets only.</small></div>
      </section>

      <section id="engine" className="workspace shell">
        <div className="panel builder">
          <div className="panel-head"><span>01</span><h2>Set intent</h2></div>
          <label>USDC allocation <small>Balance: {Number(balance).toFixed(2)}</small></label>
          <div className="amount-wrap"><input value={amount} onChange={(e) => { setAmount(e.target.value); setQuote(""); }} inputMode="decimal" /><span>USDC</span></div>
          <label>Risk policy</label>
          <div className="risk-grid">{(["Conservative", "Balanced", "Aggressive"] as HorrisRisk[]).map((item) => <button key={item} className={risk === item ? "risk active" : "risk"} onClick={() => { setRisk(item); setQuote(""); }}>{item}</button>)}</div>
          <button className="button primary" disabled={busy} onClick={reviewStrategy}>{busy ? "Checking…" : "Review live strategy"}</button>
          <p className="status">{status}</p>
        </div>

        <div id="policy" className="panel strategy">
          <div className="panel-head"><span>02</span><h2>Horris proposal</h2></div>
          <div className="strategy-title"><div><p>LIVE TESTNET ROUTE</p><h3>{strategy.title}</h3></div><span className="badge">{risk}</span></div>
          <p className="summary">Convert test USDC into Mento USDm through the approved Mento Router on Celo Sepolia.</p>
          <div className="metrics"><div><small>ROUTE</small><strong>USDC → USDm</strong></div><div><small>MAX EXECUTION</small><strong>{policy.maxAllocation} USDC</strong></div><div><small>POLICY</small><strong>{strategy.target}</strong></div></div>
          <div className="policy-box">
            <div><span className="check">✓</span><p><strong>Asset policy</strong><small>Input USDC and output USDm are explicitly defined.</small></p></div>
            <div><span className="check">✓</span><p><strong>Target policy</strong><small>Execution is built by the official Mento SDK.</small></p></div>
            <div><span className="check">✓</span><p><strong>Risk policy</strong><small>{strategy.maxDrawdown} profile · {policy.slippage}% transaction slippage ceiling.</small></p></div>
          </div>
          {quote && <div className="policy-box"><div><span className="check">→</span><p><strong>{amount} USDC ≈ {Number(quote).toFixed(4)} USDm</strong><small>Live testnet quote. Final output is protected by the selected slippage policy.</small></p></div><button className="button primary" disabled={busy} onClick={executeStrategy}>{busy ? "Executing…" : "Execute on Celo"}</button></div>}
        </div>
      </section>

      <section id="activity" className="shell activity">
        <div className="activity-head"><div><p className="eyebrow">EXECUTION LOG</p><h2>Every action stays inspectable.</h2></div><span>{txHash ? "1 TESTNET EXECUTION" : "0 EXECUTIONS"}</span></div>
        <div className="empty-state"><span>{txHash ? "✓" : "→"}</span><div><strong>{txHash ? "Transaction submitted" : "No execution yet"}</strong><p>{txHash ? <a href={`${CELO_SEPOLIA_EXPLORER}/tx/${txHash}`} target="_blank" rel="noreferrer">View transaction on Celo Sepolia explorer ↗</a> : "Connect a wallet, review the live quote, then explicitly approve execution."}</p></div></div>
      </section>
