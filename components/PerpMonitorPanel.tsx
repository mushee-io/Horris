...
export default function PerpMonitorPanel({ account }: { account?: Address }) {
  ...
  function hasMatchingPosition(order: Order) { return positions.some((position) => position.marketToken.toLowerCase() === order.marketToken.toLowerCase() && position.side === order.side); }
  function isPendingIncrease(order: Order) { return [2, 3, 8].includes(order.orderType); }
  function isOrphanStop(order: Order) { return order.orderType === 6 && !hasMatchingPosition(order); }
  function cancellationReason(order: Order) { if (isOrphanStop(order)) return "orphan-stop"; if (order.isFrozen) return "frozen-order"; if (isPendingIncrease(order)) return "pending-increase"; return "manual-review"; }
  ...
  async function refresh() {
    ...
    } catch (error) { setPositions([]); setOrders([]); setRiskState(undefined); setPreview(undefined); setCancelPreview(undefined); setStatus(error instanceof Error ? error.message : "UpDown risk-state read failed"); }
  ...
  async function compileProtection(position: Position, kind: "stop-loss" | "take-profit") {
    ...
    try {
      ...
      const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Protection preview failed");
    ...
    } catch (error) { setStatus(error instanceof Error ? error.message : "Protection preview failed"); } finally { setBusy(false); }
  }

  async function compileCancellation(order: Order) {
    if (!account) return;
    const reason = cancellationReason(order);
    setBusy(true); setCancelPreview(undefined); setPreview(undefined); setStatus(`Re-reading ${order.market} order and compiling ${reason} cancellation…`);
    try {
      ...
      const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Cancellation preview failed");
      setCancelPreview(data); setStatus("Unsigned UpDown cancellation compiled from fresh pending-order state ✓ · signing disabled");
    } catch (error) { setStatus(error instanceof Error ? error.message : "Cancellation preview failed"); } finally { setBusy(false); }
  }
  ...
  return <section className="perp-monitor shell" aria-label="UpDown risk monitor">
    ...
    {!orders.length ? <div className="empty-state compact"><span>·</span><div><strong>No pending orders loaded</strong><p>Protective, increase, decrease and limit orders will appear here when present.</p></div></div> : <div className="order-grid">{orders.map((order) => { const orphanStop = isOrphanStop(order); const recoverable = order.isFrozen || isPendingIncrease(order) || orphanStop; return <article className={order.isFrozen || orphanStop ? "order-card frozen" : "order-card"} key={order.key}><div><small>{order.market}</small><strong>{order.type}</strong></div><dl><div><dt>Side</dt><dd>{order.side.toUpperCase()}</dd></div><div><dt>Size</dt><dd>${Number(order.sizeUsd).toFixed(2)}</dd></div><div><dt>Trigger</dt><dd>{order.triggerPrice ? Number(order.triggerPrice).toFixed(6) : "—"}</dd></div><div><dt>Acceptable</dt><dd>{order.acceptablePrice ? Number(order.acceptablePrice).toFixed(6) : "—"}</dd></div><div><dt>Fee</dt><dd>{Number(order.executionFeeCelo).toFixed(4)} CELO</dd></div><div><dt>Status</dt><dd>{order.isFrozen ? "FROZEN" : orphanStop ? "ORPHAN STOP" : "PENDING"}</dd></div></dl>{recoverable && <button className="recovery-button" disabled={busy} onClick={() => compileCancellation(order)}>Preview {orphanStop ? "stale stop" : "cancellation"}</button>}<p>{order.updatedAt ? `Updated ${new Date(order.updatedAt * 1000).toLocaleString()}` : order.key}</p></article>; })}</div>}
