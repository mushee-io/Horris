...
type Protection = {
  market: string; side: "long" | "short"; sizeUsd: number; effectiveLeverage: number | null; leverageWithinPolicy: boolean;
  stopOrderCount: number; stopCoveragePercent: number; fullyStopProtected: boolean; hasFrozenStop: boolean; pendingIncreaseUsd: number;
};
...
type ProtectionPreview = {
  protection: { kind: "stop-loss" | "take-profit"; human: { market: string; sizeUsd: number; triggerPrice: number; acceptablePrice: number } };
  liveExecutionFee: { bufferedFeeCelo: string };
  unsignedTransaction: { calldataHash: `0x${string}`; calls: string[]; value: string; executionEnabled: false };
  compiledAt: string;
};
...
export default function PerpMonitorPanel({ account }: { account?: Address }) {
  ...
  async function refresh() {
    if (!account) {
      setPositions([]); setOrders([]); setRiskState(undefined); setPreview(undefined);
      setStatus("Connect a wallet to monitor UpDown positions and orders"); return;
    }
  ...
  async function compileProtection(position: Position, kind: "stop-loss" | "take-profit") {
    ...
    setBusy(true); setPreview(undefined); setStatus(`Re-reading ${position.market} and compiling ${kind} protection…`);
    try {
      const response = await fetch("/api/perps/protection-preview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ account, marketToken: position.marketToken, side: position.side, kind, triggerPrice, acceptablePriceSlippageBps: 100 }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Protection preview failed");
      setPreview(data);
      setStatus(`${kind === "stop-loss" ? "Stop-loss" : "Take-profit"} preview compiled from fresh UpDown position state ✓ · signing disabled`);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Protection preview failed");
    } finally { setBusy(false); }
  }
