...
interface IUpDownExchangeRouterShape {
    struct OrderAddresses { address receiver; address cancellationReceiver; address callbackContract; address uiFeeReceiver; address market; address initialCollateralToken; address[] swapPath; }
    struct OrderNumbers { uint256 sizeDeltaUsd; uint256 initialCollateralDeltaAmount; uint256 triggerPrice; uint256 acceptablePrice; uint256 executionFee; uint256 callbackGasLimit; uint256 minOutputAmount; uint256 validFromTime; }
    struct CreateOrderParams { OrderAddresses addresses; OrderNumbers numbers; uint8 orderType; uint8 decreasePositionSwapType; bool isLong; bool shouldUnwrapNativeToken; bool autoCancel; bytes32 referralCode; }
...
contract HorrisUpDownCalldataGuard is HorrisOwnable2Step {
    uint32 public constant BPS = 10_000;
    uint32 public constant ABSOLUTE_MAX_LEVERAGE_BPS = 100_000;
    uint256 private constant USDT_TO_USD_E30 = 1e24;
    ...
    constructor(address exchangeRouter_, address orderVault_, address usdt_, uint32 initialMaxLeverageBps, uint256 initialMaxNotionalUsdE30) {
    ...
    function setMarket(address market, bool allowed) external onlyOwner { require(market != address(0), "ZERO_MARKET"); allowedMarkets[market] = allowed; emit MarketUpdated(market, allowed); }
    function setLimits(uint32 leverageBps, uint256 notionalUsdE30) external onlyOwner { _setLimits(leverageBps, notionalUsdE30); }

    function inspectIncrease(address target, address expectedReceiver, uint256 msgValue, bytes calldata multicallData) external view returns (Inspection memory result) {
        ...
        (address collateralToken, address tokenReceiver, uint256 collateralAmount) = abi.decode(_arguments(calls[1]), (address, address, uint256));
        ...
        IUpDownExchangeRouterShape.CreateOrderParams memory params = abi.decode(_arguments(calls[2]), (IUpDownExchangeRouterShape.CreateOrderParams));
        ...
        result = Inspection({ market: params.addresses.market, collateralUsdt: collateralAmount, notionalUsdE30: params.numbers.sizeDeltaUsd, leverageBps: uint32(leverage), executionFee: wntAmount, isLong: params.isLong });
    }
    ...
    function _selector(bytes memory data) internal pure returns (bytes4 selector) { require(data.length >= 4, "SHORT_INNER_CALL"); assembly { selector := mload(add(data, 32)) } }
