...
/// @notice Narrow adapter that permits a Horris vault to execute only configured Mento stablecoin routes.
contract HorrisMentoAdapter {
    ...
    constructor(address vault_, address router_, address tokenIn_, address tokenOut_) {
        require(vault_ != address(0) && router_ != address(0), "ZERO_ADDRESS");
        vault = vault_;
        router = router_;
        tokenIn = tokenIn_;
        tokenOut = tokenOut_;
    ...
    function executeSwap(
        uint256 amountIn,
        uint256 amountOutMin,
        IMentoRouter.Step[] calldata path,
        uint256 deadline
        ...
        uint256 afterOut = IERC20Adapter(tokenOut).balanceOf(address(this));
